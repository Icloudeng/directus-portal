...
import { DotButton } from '@/components/ui/carouselButtons/CarouselButtons';
...
export const Accordion: FunctionComponent<PropsWithChildren<Props>> = ({
  ...
  const defaultKey = defaultSelected
    ? '.$' + defaultSelected
    : (arrChildren[0] as React.ReactElement)?.key;
    ...
    <div className='accordion__top-container pt-5 w-full'>
      <div className='relative w-full accordion__main-container'>
            ...
            <div className='accordion__list-contents w-full text-content flex-[1.3] flex flex-col items-start gap-7 sm:border-l-[1px]'>
              {titles?.map(({ title, description, key }) => {
                ...
                return (
                  <div
                    key={key}
                    className={`transition-all accordion__list-contents-texts w-full flex-1 ${
                      active
                        ? 'flex sm:border-l-[1px] border-primary-400'
                        : 'hidden sm:flex'
                    } flex-col items-center sm:items-start gap-5 pl-7`}
                  >
                    <h3
                    ...
                    <span
                      className={`transition-all sm:max-w-sm text-center sm:text-start ${
                        !active && 'hidden'
                      }`}
                    >
                      {description}
        ...
        <div className='w-full flex sm:hidden items-center justify-between ss:justify-evenly float-right mt-10'>
          ...
          >
            <span
              className={`inline-flex items-center justify-center w-8 h-8 rounded-full sm:w-10 sm:h-10 bg-white/30 group-hover:bg-white/50 ring-1 group-focus:ring-3 group-focus:ring-primary-400 group-focus:outline-none ${
                !hasPrev && 'ring-gray-200 '
              }`}
            >
              <svg
                aria-hidden='true'
                className={`w-5 h-5 text-primary-400 sm:w-6 sm:h-6 ${
                  !hasPrev && 'text-gray-200 '
                }`}
                fill='none'
                stroke='currentColor'
                viewBox='0 0 24 24'
          ...
          >
            <span
              className={`inline-flex items-center justify-center w-8 h-8 rounded-full sm:w-10 sm:h-10 bg-white/30 group-hover:bg-white/50 ring-1 group-focus:ring-3 group-focus:ring-primary-400 group-focus:outline-none ${
                !hasNext && 'ring-gray-200 '
              }`}
            >
              <svg
                aria-hidden='true'
                className={`w-5 h-5 text-primary-400 sm:w-6 sm:h-6 ${
                  !hasNext && 'text-gray-200 '
                }`}
                fill='none'
                stroke='currentColor'
                viewBox='0 0 24 24'
...
export function AccordionChild({
  ...
  return (
    <div className={`transition-all ${!active ? 'hidden' : ''}`}>
      {children}
    </div>
  );
}
