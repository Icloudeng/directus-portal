...
function STHorizontalTimeline(props: { item: ST_HorizontalTimeline }) {
  ...
  return (
    ...
    >
      <div
        style={{ background: item.color }}
        className='w-5 h-5  absolute -left-10 transform -translate-x-2/4 rounded-full z-10 mt-2 md:mt-0'
      ></div>

      <div
        style={{ background: item.color }}
        className='w-10 h-1  absolute -left-10 z-0'
      ></div>
      ...
      <a
        href='{item.link}'
        className='text-center border bg-transparent hover:bg-blue-500  font-semibold hover:text-white py-2 px-4 border border-blue-500 hover:border-transparent rounded  text-white hover:text-gray-300'
      >
        {item.translations?.link_text}
      </a>
    </div>
  );
}
