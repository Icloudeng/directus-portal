...
export const DotButton = ({ position, selected, onClick }: IDotButton) => {
    return (
        <button
            type='button'
            className={`w-3 h-3 rounded-full ${selected ? 'bg-primary-300' : 'bg-gray-300/50 hover:bg-gray-300'}`}
...
export const PrevButton = ({ enabled, onClick }: ICarouselButton) => {
    return (
        <button
            ...
            data-carousel-prev=''
        >
            <span className={`inline-flex items-center justify-center w-8 h-8 rounded-full sm:w-10 sm:h-10 bg-white/30 group-hover:bg-white/50 ring-2 group-focus:ring-3 group-focus:ring-primary-400 group-focus:outline-none ${!enabled && 'ring-gray-200 '}`}>
                <svg aria-hidden='true' className={`w-5 h-5 text-primary-400 sm:w-6 sm:h-6 ${!enabled && 'text-gray-200 '}`} fill='none' stroke='currentColor' viewBox='0 0 24 24' xmlns='http://www.w3.org/2000/svg'>
